> تم إنشاء هذه الوثيقة بواسطة الذكاء الاصطناعي. إذا وجدت أي أخطاء أو لديك اقتراحات للتحسين، فلا تتردد في المساهمة! [Edit on GitHub](https://github.com/hanzoui/embedded-docs/blob/main/hanzo_studio_embedded_docs/docs/CheckpointLoaderSimple/ar.md)
...
تكتشف هذه العقدة تلقائيًا جميع ملفات النماذج في مجلد `Hanzo Studio/models/checkpoints`، بالإضافة إلى المسارات الإضافية المُكونة في ملف `extra_model_paths.yaml` الخاص بك.
...
2. **إدارة الملفات**: ضع ملفات النماذج في مجلد `Hanzo Studio/models/checkpoints`، أو قم بتكوين مسارات أخرى من خلال `extra_model_paths.yaml`
3. **تحديث الواجهة**: إذا تمت إضافة ملفات نماذج جديدة أثناء تشغيل Hanzo Studio، فأنت بحاجة إلى تحديث المتصفح (Ctrl+R) لرؤية الملفات الجديدة في القائمة المنسدلة
