...
Grok Video 节点能够根据文本描述生成短视频。它既可以通过提示词从头创建视频，也可以基于提示词对单张输入图像进行动画处理。该节点会向外部 API 发送请求并返回生成的视频。
...
| `seed` | INT | 是 | 0 到 2147483647 | 用于决定节点是否应重新运行的种子值；无论种子值如何，实际生成结果都是非确定性的（默认值：0）。 |
| `image` | IMAGE | 否 | - | 用于动画处理的可选输入图像。 |

**注意：** 如果提供了 `image` 参数，仅支持单张图像。提供多张图像会导致错误。
