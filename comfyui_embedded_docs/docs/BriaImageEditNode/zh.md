...
Bria FIBO 图像编辑节点允许您通过文本指令修改现有图像。它会将图像和您的提示词发送至 Bria API，该 API 使用 FIBO 模型根据您的要求生成新的、经过编辑的图像版本。您还可以提供一个遮罩，以将编辑限制在特定区域。
...
| `structured_prompt` | STRING | 否 | - | 一个包含 JSON 格式结构化编辑提示的字符串。使用此参数代替常规提示词，以实现精确的程序化控制（默认：空）。 |
| `seed` | INT | 是 | 1 至 2147483647 | 用于初始化随机生成的数字，确保结果可复现（默认：1）。 |
| `guidance_scale` | FLOAT | 是 | 3.0 至 5.0 | 控制生成图像遵循提示词的紧密程度。值越高，遵循度越强（默认：3.0）。 |
...
| `structured_prompt` | STRING | 在编辑过程中使用或生成的结构化提示词。 |