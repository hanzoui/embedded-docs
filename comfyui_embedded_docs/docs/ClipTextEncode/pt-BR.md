...
`CLIP Text Encode (CLIPTextEncode)` atua como um tradutor, convertendo suas descrições de texto em um formato que a IA possa entender. Isso ajuda a IA a interpretar sua entrada e gerar a imagem desejada.
...
Exemplo: Se você tiver um modelo chamado `EasyNegative.pt` em sua pasta `ComfyUI/models/embeddings`, poderá usá-lo assim:
...
Se você quiser incluir parênteses literais em seu prompt sem alterar o peso, pode escapá-los usando uma barra invertida, por exemplo: `\(palavra\)`.

### *Wildcards*/Prompts Dinâmicos
...
Se você quiser incluir chaves literais em seu prompt sem acionar o comportamento dinâmico, pode escapá-las usando uma barra invertida, por exemplo: `\{palavra\}`.
