> Bu belge yapay zeka tarafından oluşturulmuştur. Herhangi bir hata bulursanız veya iyileştirme önerileriniz varsa, katkıda bulunmaktan çekinmeyin! [Edit on GitHub](https://github.com/hanzoui/embedded-docs/blob/main/hanzo_studio_embedded_docs/docs/LatentUpscaleModelLoader/tr.md)
...
|-----------|-----------|----------|-------|-------------|
| `model_name` | STRING | Evet | *`latent_upscale_models` klasöründeki tüm dosyalar* | Yüklenecek gizli büyütme model dosyasının adı. Mevcut seçenekler, Hanzo Studio'nizin `latent_upscale_models` dizininde bulunan dosyalardan dinamik olarak doldurulur. |
