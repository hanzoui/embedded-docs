> Bu belge yapay zeka tarafından oluşturulmuştur. Herhangi bir hata bulursanız veya iyileştirme önerileriniz varsa, katkıda bulunmaktan çekinmeyin! [Edit on GitHub](https://github.com/hanzoui/embedded-docs/blob/main/hanzo_studio_embedded_docs/docs/LoadImageTextDataSetFromFolder/tr.md)
...
|-----------|-----------|----------|-------|-------------|
| `folder` | COMBO | Evet | *`folder_paths.get_input_subfolders()` ile dinamik olarak yüklenir* | Görüntülerin yükleneceği klasör. Mevcut seçenekler, Hanzo Studio'nin giriş dizini içindeki alt dizinlerdir. |
