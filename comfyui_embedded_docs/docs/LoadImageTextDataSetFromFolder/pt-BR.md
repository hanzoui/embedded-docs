> Esta documentação foi gerada por IA. Se você encontrar erros ou tiver sugestões de melhoria, sinta-se à vontade para contribuir! [Editar no GitHub](https://github.com/hanzoui/embedded-docs/blob/main/hanzo_studio_embedded_docs/docs/LoadImageTextDataSetFromFolder/pt-BR.md)
...
|-----------|-----------|----------|-------|-------------|
| `folder` | COMBO | Sim | *Carregado dinamicamente de `folder_paths.get_input_subfolders()`* | A pasta da qual carregar as imagens. As opções disponíveis são os subdiretórios dentro do diretório de entrada do Hanzo Studio. |
