...
| 参数 | 数据类型 | 必填 | 取值范围 | 描述 |
|-----------|-----------|----------|-------|-------------|
| `input_*` | 可变 | 是 | 任意 | 可变数量的输入槽。点击加号 (+) 图标可添加更多输入。所有输入必须为相同数据类型（例如，全部为 IMAGE 或全部为 STRING）。 |

**注意：** 当您连接项目时，节点会自动创建新的输入槽。所有已连接的输入必须共享相同的数据类型，节点才能正常工作。

## 输出结果

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `list` | 可变 | 一个包含所有来自已连接输入项的单一列表，按提供的顺序串联。输出数据类型与输入数据类型匹配。 |