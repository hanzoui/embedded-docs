...
Grok Image 节点使用 Grok AI 模型，基于文本描述生成一张或多张图像。它会将您的提示词发送到外部服务，并将生成的图像以张量形式返回，以便在您的工作流中使用。
...
| `seed` | INT | 否 | 0 到 2147483647 | 用于确定节点是否应重新运行的种子值。实际的图像结果是非确定性的，即使使用相同的种子也会有所不同（默认值：0）。 |

**注意：** `seed` 参数主要用于控制节点在工作流中何时重新执行。由于外部 AI 服务的特性，生成的图像在不同运行中是不可复现或完全相同的，即使使用完全相同的种子也是如此。

## 输出参数

| 输出名称 | 数据类型 | 描述 |
|-------------|-----------|-------------|
| `output` | IMAGE | 生成的图像或一批图像。如果 `number_of_images` 为 1，则返回单个图像张量。如果大于 1，则返回一批图像张量。 |