Le nœud Preview3DAnimation est principalement utilisé pour prévisualiser la sortie des modèles 3D. Ce nœud prend deux entrées : l'une est le `camera_info` du nœud Load3D, l'autre est le chemin du fichier du modèle 3D. Le chemin du fichier du modèle doit se trouver dans le dossier `HanzoStudio/output`.
...
| model_file       | STRING  | Chemin du fichier du modèle dans `HanzoStudio/output/`                 |
