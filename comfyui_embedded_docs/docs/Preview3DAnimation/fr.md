Le nœud Preview3DAnimation est principalement utilisé pour prévisualiser la sortie des modèles 3D. Ce nœud prend deux entrées : l'une est le `camera_info` du nœud Load3D, l'autre est le chemin du fichier du modèle 3D. Le chemin du fichier du modèle doit se trouver dans le dossier `Hanzo Studio/output`.
...
| model_file       | STRING  | Chemin du fichier du modèle dans `Hanzo Studio/output/`                 |

## Description de la zone Canevas (Canvas)

Actuellement, les nœuds liés à la 3D dans l’interface Hanzo Studio partagent le même composant de canevas, donc leurs opérations de base sont pour la plupart cohérentes, à l’exception de quelques différences fonctionnelles.
