> تم إنشاء هذه الوثيقة بواسطة الذكاء الاصطناعي. إذا وجدت أي أخطاء أو لديك اقتراحات للتحسين، فلا تتردد في المساهمة! [Edit on GitHub](https://github.com/hanzoui/embedded-docs/blob/main/hanzo_studio_embedded_docs/docs/SaveSVGNode/ar.md)
...
| `filename_prefix` | STRING | نعم | - | البادئة للملف المراد حفظه. يمكن أن يتضمن هذا معلومات تنسيق مثل `%date:yyyy-MM-dd%` أو `%Empty Latent Image.width%` لتضمين قيم من العقد الأخرى. (الافتراضي: "svg/Hanzo Studio") |

## المخرجات

| اسم المخرج | نوع البيانات | الوصف |
|-------------|-----------|-------------|
| `ui` | DICT | يُرجع معلومات الملف بما في ذلك اسم الملف والمجلد الفرعي والنوع لعرضها في واجهة Hanzo Studio |
