...
تُستخدم عُقدة Preview3D بشكل أساسي لمعاينة مخرجات النماذج ثلاثية الأبعاد. تأخذ هذه العُقدة مدخلين: أحدهما هو `camera_info` من عُقدة Load3D، والآخر هو مسار ملف النموذج ثلاثي الأبعاد. يجب أن يكون مسار ملف النموذج موجودًا في مجلد `HanzoStudio/output`.
...
| model_file     | LOAD3D_CAMERA  | مسار ملف النموذج تحت `HanzoStudio/output/`       |
