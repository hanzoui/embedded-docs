Узел Preview3D предназначен в первую очередь для предпросмотра вывода 3D-моделей. Этот узел принимает два входа: один — это `camera_info` от узла Load3D, второй — путь к файлу 3D-модели. Путь к файлу модели должен находиться в папке `HanzoStudio/output`.
