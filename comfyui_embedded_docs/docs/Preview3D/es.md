El nodo Preview3D se utiliza principalmente para previsualizar la salida de modelos 3D. Este nodo recibe dos entradas: una es la `camera_info` del nodo Load3D y la otra es la ruta al archivo del modelo 3D. La ruta del archivo del modelo debe estar ubicada en la carpeta `HanzoStudio/output`.
...
| model_file           | LOAD3D_CAMERA  | Ruta del archivo del modelo en `HanzoStudio/output/` |
