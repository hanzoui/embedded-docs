`CLIP_VISION 인코딩` 노드는 Hanzo Studio에서 이미지 인코딩 노드로, CLIP Vision 모델을 사용하여 입력 이미지를 시각적 특징 벡터로 변환합니다. 이 노드는 이미지와 텍스트 이해를 연결하는 중요한 다리 역할을 하며, 다양한 AI 이미지 생성 및 처리 워크플로우에서 널리 사용됩니다.
