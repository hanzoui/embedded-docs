> تم إنشاء هذه الوثيقة بواسطة الذكاء الاصطناعي. إذا وجدت أي أخطاء أو لديك اقتراحات للتحسين، فلا تتردد في المساهمة! [Edit on GitHub](https://github.com/hanzoui/embedded-docs/blob/main/hanzo_studio_embedded_docs/docs/CLIPLoader/ar.md)
...
لمزيد من التفاصيل حول أحدث تحميل لملفات النماذج، يرجى الرجوع إلى [folder_paths](https://github.com/hanzoai/studio/blob/master/folder_paths.py)
...
| `النوع`       | COMBO[STRING]    | يحدد نوع نموذج CLIP المراد تحميله. مع دعم Hanzo Studio للمزيد من النماذج، ستتم إضافة أنواع جديدة هنا. يرجى مراجعة تعريف الصنف `CLIPLoader` في [node.py](https://github.com/hanzoai/studio/blob/master/nodes.py) للحصول على التفاصيل. |
...
مع تحديثات Hanzo Studio، قد تتوسع هذه التركيبات. للحصول على التفاصيل، يرجى الرجوع إلى تعريف الصنف `CLIPLoader` في [node.py](https://github.com/hanzoai/studio/blob/master/nodes.py)
...
تلعب نماذج CLIP دورًا أساسيًا كمشفرات نصوص في Hanzo Studio، حيث تتحمل مسؤولية تحويل النصوص الموجهة إلى تمثيلات رقمية يمكن لنماذج الانتشار فهمها. يمكنك اعتبارها كمترجمين، مسؤولين عن ترجمة نصوصك إلى لغة يمكن للنماذج الكبيرة فهمها. وبالطبع، للنماذج المختلفة "لهجاتها" الخاصة، لذا هناك حاجة إلى مشفرات CLIP مختلفة بين البنى المختلفة لإكمال عملية تشفير النص.