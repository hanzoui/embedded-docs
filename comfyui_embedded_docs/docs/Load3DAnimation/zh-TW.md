...
Load3DAnimation 節點是用於載入和處理 3D 模型檔案的核心節點。載入節點時，它會自動從 `HanzoStudio/input/3d/` 中獲取可用的 3D 資源。您也可以使用上傳功能上傳支援的 3D 檔案進行預覽。
...
| 參數名稱      | 類型           | 描述                                                     | 預設值 | 範圍          |
|--------------|----------------|----------------------------------------------------------|---------|--------------|
| `模型檔案` | File Selection | 3D 模型檔案路徑，支援上傳，預設從 `HanzoStudio/input/3d/` 讀取模型檔案 | -       | 支援的格式    |
