...
| 파일명 접두사 | STRING | 예 | "clip/Hanzo Studio" | 저장할 파일의 접두사 경로 |
| prompt | PROMPT | 숨김 | - | 워크플로우 프롬프트 정보 (메타데이터용) |
| extra_pnginfo | EXTRA_PNGINFO | 숨김 | - | 추가 PNG 정보 (메타데이터용) |

## 출력

이 노드는 정의된 출력 유형이 없습니다. 처리된 파일은 `Hanzo Studio/output/` 폴더에 저장됩니다.
...
1. **파일 위치**: 모든 파일은 `Hanzo Studio/output/` 디렉토리에 저장됩니다
