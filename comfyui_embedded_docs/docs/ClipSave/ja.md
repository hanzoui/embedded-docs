...
| ファイル名プレフィックス | STRING | はい | "clip/Hanzo Studio" | 保存するファイルのプレフィックスパス |
| prompt | PROMPT | 非表示 | - | ワークフローのプロンプト情報（メタデータ用） |
| extra_pnginfo | EXTRA_PNGINFO | 非表示 | - | 追加のPNG情報（メタデータ用） |

## 出力

このノードには定義された出力タイプはありません。処理されたファイルは`Hanzo Studio/output/`フォルダに保存されます。
...
1. **ファイルの場所**: すべてのファイルは`Hanzo Studio/output/`ディレクトリに保存されます
